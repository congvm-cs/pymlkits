...
DEFAULT_MODEL_FNAME = 'version-RFB-320-optimized-graph.onnx'


class LightweightFaceDetector():
    def __init__(self, model_path=None, gpu_id=-1, mem_fraction=0.2, input_size=(320, 240), intra_op_num_threads=1):
        self.model_path = model_path or os.path.join(CWD, DEFAULT_MODEL_FNAME)
        self.gpu_id = gpu_id
        self.image_mean = np.array([127, 127, 127])
        self.std = 128.0
        self.intra_op_num_threads = intra_op_num_threads
        self.input_size = input_size
        self._load_model(self.model_path, gpu_id)
    ...
    def detect(self, img_arr, threshold=0.8):
        ...
        ----------
        bboxes: ndarray
            The 2D array of bounding boxes
        labels: int
            Labels of a content in the bounding box
        probs: face score
            The probability of face
        """
